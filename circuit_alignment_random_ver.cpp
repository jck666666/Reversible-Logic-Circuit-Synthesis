...
void randrepair()
{
    ...
    {
        for (int j = 0; j < m; j++) // gates
        {
            int cnt = 0;
            for (int k = 0; k < n; k++) // component
            {
                if (x[i][k][j] == 3)
                {
                    cnt++;
            ...
            while (cnt > 1)
            {
                for (int k = 0; k < n; k++) // component
                {
                    if (x[i][k][j] == 3)
                    {
                        x[i][k][j] = 0 + (double)(rand() * (2.99 - 0)) / (double)RAND_MAX;
                        cnt--;
                    }
                }
            }
...
int main()
{
    ...
    for (int time = 0; time < test; time++)
    {
        ...
        w2 = 0;
        best = 0, worst = 0; // population of best and worst
        b = 0.0, w = 100;
    }
...
void oldfitness()
{
    double fit1[population] = {0.0};
    double fit2[population] = {0.0};
    ...
    for (int i = 0; i < population; i++)
    {
        ...
        fit1[i] = (double)COP / (double)pow(2, n);
        if (fit1[i] != 1)
        {
            allRight = false;
        }
        ...
        {
            fit2[i] = (double)WG / (double)m;
        }
    }

    // allRight ? w2 = 0.4 : w2 = 0;
    if (allRight)
    {
        w2 = 0.4;
    }
 ...
 //   cout << "zero = " << cntzero << endl;
    ...
    for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < m; j++)
        {
            ...
            for (int r = 0; r < cnt; r++)
            {
                if (gb[i][j] != x[r][i][j]) // have to update
                {
                    Q[i][j][gb[i][j]] += delta;
                    Q[i][j][x[r][i][j]] -= delta;
                }
